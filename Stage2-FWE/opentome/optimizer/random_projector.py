...
class GradientProjector:
    ...
    def project(self, full_rank_grad: torch.Tensor, iter: int) -> torch.Tensor:
        ...
        """
        need_update = not self._ortho_matches(full_rank_grad) or iter % self.update_proj_gap == 0
    ...
    def state_dict(self):
        ...
        if ortho is None:
            return {"has_ortho": False, "seed": self.seed, "svd_count": self.svd_count}
        return {"has_ortho": True, "ortho": ortho.cpu(), "seed": self.seed, "svd_count": self.svd_count}

    def load_state_dict(self, state):
        self.seed = state["seed"]
        self.svd_count = state.get("svd_count", 0)
        self.ortho_matrix = state["ortho"] if state["has_ortho"] else None

    def _ortho_matches(self, full_rank_grad: torch.Tensor) -> bool:
        """Return False if cached ortho_matrix has wrong shape for current grad."""
        ...
        if self.proj_type in ("std", "right"):
            if g.shape[0] >= g.shape[1]:
                return o.shape == (self.rank, g.shape[1])
            else:
                return o.shape == (g.shape[0], self.rank)
        elif self.proj_type == "reverse_std":
            if g.shape[0] >= g.shape[1]:
                return o.shape == (g.shape[0], self.rank)
            else:
