...
class AdamW(Optimizer):
    ...
    def load_state_dict(self, state_dict):
        ...
        for param_id, state in state_dict["state"].items():
            ...
            if proj_keys:
                projector_states[param_id] = {k[len("__proj_"):]: state.pop(k) for k in proj_keys}
        super().load_state_dict(state_dict)
        all_params = [p for group in self.param_groups for p in group["params"]]
        for param_id, proj_sd in projector_states.items():
            p = all_params[int(param_id)]
            live_state = self.state.get(p, {})
            if "projector" not in live_state:
                live_state["projector"] = GradientProjector(
                    self.param_groups[0].get("rank", 256),
                    seed=live_state.get("seed", 0),
                )
    ...
    def step(self, closure: Callable = None):
        ...
        for group in self.param_groups:
            for p in group["params"]:
                ...
                # Skip 1D params (bias, norm weights) — they have no second dim.
                # Skip 3D+ params (e.g. deltanet state tensors) — projector assumes 2D.
                if "rank" in group and grad.dim() == 2:
                ...
                if "rank" in group and p.grad.dim() == 2:
