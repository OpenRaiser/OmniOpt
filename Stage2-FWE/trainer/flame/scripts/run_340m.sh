#!/bin/bash
# Stage 2 — FineWeb-Edu 340M, best hyperparameters per optimizer
# 4 archs: transformer / gla / deltanet / gated_deltanet
# 8 GPUs, 1 node | 30720 steps | seq_len=32768 | grad_acc=2
#
# Usage: bash scripts/run_340m.sh <arch> <optimizer>
# Example: bash scripts/run_340m.sh gla muon
...
case $ARCH in
  transformer)    MODEL_CONFIG="configs/transformer_340M.json" ;;
  gla)            MODEL_CONFIG="configs/gla_340M.json" ;;
  deltanet)       MODEL_CONFIG="configs/delta_net_340M.json" ;;
  gated_deltanet) MODEL_CONFIG="configs/gated_deltanet_340M.json" ;;
  *)
...
case $OPT in
  ...
  adan)
    OPT_NAME=Adan  LR=1e-3 B1=0.9 B2=0.92 EPS=1e-8
    export OPT_BETA3=0.99 ;;
  lion)
    OPT_NAME=Lion  LR=3e-4 B1=0.9 B2=0.99 EPS="" ;;
  mars_adamw)
    OPT_NAME=MARS  LR=1e-3 B1=0.95 B2=0.99 EPS=1e-8
    export MARS_TYPE=mars-adamw ;;
  mars_lion)
    OPT_NAME=MARS  LR=2e-4 B1=0.9  B2=0.98 EPS=1e-12
    export MARS_TYPE=mars-lion ;;
  mars_shampoo)
    OPT_NAME=MARS  LR=1e-2 B1=0.95 B2=0.99 EPS=1e-12
    export MARS_TYPE=mars-shampoo ;;
  muon)
    OPT_NAME=Muon  LR=3e-3 B1=0.9  B2=0.95 EPS=1e-15 ;;
  rmnp)
    OPT_NAME=RMNP  LR=3e-3 B1=0.9  B2=0.99 EPS=1e-15
    export ADAM_LR=1e-3 ;;
  soap)
  ...
  conda)
    OPT_NAME=Conda LR=1e-2 B1=0.9 B2=0.99 EPS=1e-12
    export CONDA_RANK=256 CONDA_SCALE=0.25 CONDA_UPDATE_PROJ_GAP=2000 CONDA_PROJ_TYPE=std ;;
  *)
...
EXPERIMENT_NAME="${ARCH}_340m_fwe_${OPT}"
