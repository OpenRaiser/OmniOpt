...
def main(job_config: JobConfig):
    ...
    if val_data_dir is not None:    # User explicitly specified val data path
        if os.path.isfile(val_data_dir):    # Directly passed file path (e.g. parquet)
            ...
            val_parquet = val_data_dir
        else:    # Passed is a directory, auto-detect C4 or parquet
            c4_glob = os.path.join(val_data_dir, "c4-validation.*.json.gz")
            import glob as _glob
            if _glob.glob(c4_glob):
                val_parquet = c4_glob
            else:
    ...
    # For Transformer models (with max_position_embeddings), use context_len to avoid
    # position ID overflow beyond max_position_embeddings. For linear recurrent models
    # (no position embedding limit), use full seq_len for richer context.
    C4_TARGET_EVAL_TOKENS = 10_000_000
    val_seq_len = (
    ...
    logger.info(f"{color.yellow}Pre-caching val chunks from {val_parquet} (val_seq_len={val_seq_len}) ...{color.reset}")
    ...
    logger.info(
        ...
        f"({len(val_chunks_cache) * val_seq_len:,} tokens per rank){color.reset}"
    )
    ...
    # ------ jinxin ------ #
    # Compute final validation PPL after training
    logger.info(f"Computing final validation PPL on {val_parquet}...")
