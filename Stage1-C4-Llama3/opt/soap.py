...
class SOAP(optim.Optimizer):
    ...
    def project(self, grad, state, merge_dims=False, max_precond_dim=10000):
        ...
        for mat in state['Q']:
            if len(mat) > 0:
                if grad.dtype != mat.dtype:  ### fix bug of float and bf16
                    mat = mat.type(grad.dtype)
            ...
            if merge_dims:
                ...
                for idx, sh in enumerate(new_grad.shape):
                    if sh <= max_precond_dim:
                        ...
                        ### fix bug of float and bf16
                        state['GG'][idx].lerp_((outer_product).type(state['GG'][idx].dtype), 1-state['shampoo_beta'])
