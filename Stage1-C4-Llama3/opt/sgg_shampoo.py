...
def NewtonSchulz(M, steps=5, eps=1e-7):
    ...
    norm_val = M.norm(dim=(-2, -1), keepdim=True)
    X = M.bfloat16() / (norm_val + eps)
...
class SGGShampoo(Optimizer):
    """SGGShampoo: Combines SGGAdamW's clustering with Shampoo's preconditioning."""
                ...
                if self.global_median is None:
                    ...
                    for other_group in self.param_groups:
                        for p in other_group["params"]:
                            ...
                            # grad = self._align_gradients(p, normalized_grad).view(-1)
                            grad = self._align_gradients(p, p.grad).view(-1)
                            flat_grad = grad.view(-1)
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                if state["step"] % group["scale_update_freq"] == 0 and param.dim() > 1:
                ...
                # Compute update
                if param.dim() > 1 or group["optimize_1d"]:
