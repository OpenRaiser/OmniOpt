...
class SGGAdamW(Optimizer):  # v0828
    """Memory-optimized SGGAdamW with efficient clustering integration."""
    ...
    def _compute_scale_factors(self) -> None:
        """Memory-efficient computation of group-wise scale factors using divide-and-conquer."""
        param_map = {
            id(p): p
        ...
        for group_idx, group in enumerate(self.param_groups):
            ...
            # Compute group statistics incrementally
            for p in group["params"]:
            ...
            if group_distances:
                ...
                # Compute global statistics incrementally if not set
                if self.global_median is None:
                    ...
                    for other_group in self.param_groups:
                        for p in other_group["params"]:
                            ...
                            # grad = self._align_gradients(p, normalized_grad)
                            grad = self._align_gradients(p, p.grad)
                            flat_grad = grad.view(-1)
                            global_sum += flat_grad.sum().item()
                            global_count += flat_grad.numel()
                            global_distances.append((flat_grad - (global_sum / global_count)).abs())
    ...
    def step(self, closure: Optional[Callable[[], float]] = None) -> Optional[float]:
        """Optimized step with memory-efficient clustering."""
        loss = None
        ...
        for group in self.param_groups:
            ...
            for param in group["params"]:
                ...
                # Compute bias corrections and base step size
                ...
                if (param.dim() > 1 and "clusters" in state and state["clusters"] is not None):
                    ...
                    scaled_lr = base_step_size * group_scale * scales
                    update.mul_(-scaled_lr)
                else:
                    update.mul_(-base_step_size * group_scale)
